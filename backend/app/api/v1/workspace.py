...
def create_folder(folder: WorkspaceFolderCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return WorkspaceService.create_folder(db, current_user.id, folder)
...
def create_saved_search(search: SavedSearchCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return WorkspaceService.create_saved_search(db, current_user.id, search)
