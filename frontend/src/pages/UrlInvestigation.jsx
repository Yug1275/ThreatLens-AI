...
// Display value: shows "Unavailable" for null/undefined, otherwise the value
const val = (v, suffix = '') => {
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          ...
          {activeTab === 'overview' && (
            <div className="row g-4">
              ...
              {/* Right — Domain + SSL summary cards */}
                ...
                <div className="row g-4">
                  ...
                  <div className="col-sm-6">
                    <SectionCard
                      ...
                      icon={<Globe size={16} color="var(--tl-primary-light)" />}
                      error={di.whois_error}
                    >
                  ...
                  <div className="col-sm-6">
                    <SectionCard
                      ...
                      icon={<Shield size={16} color="var(--tl-primary-light)" />}
                      error={si.ssl_error && !si.valid ? si.ssl_error : null}
                    >
                      <InfoRow label="Status">
                        {si.valid
                          ? <Badge variant="success"><CheckCircle size={11} style={{ marginRight: 4 }} />Valid</Badge>
                          : <Badge variant="danger"><XCircle size={11} style={{ marginRight: 4 }} />Invalid</Badge>
                        }
                      </InfoRow>
                      ...
                      <InfoRow label="Days Left">
                        {si.days_remaining != null
                          ? <span style={{ color: si.days_remaining < 30 ? 'var(--tl-warning)' : 'var(--tl-success)', fontWeight: 600 }}>
                              {si.days_remaining}d
                            </span>
                          : val(null)}
                      </InfoRow>
            ...
            <div className="row g-4">
              <div className="col-12 col-lg-6">
                <SectionCard
                  ...
                  icon={<Database size={16} color="var(--tl-primary-light)" />}
                  error={di.whois_error}
                >
                  ...
                  <InfoRow label="Privacy Enabled">
                    {di.whois_privacy === true
                      ? <Badge variant="warning">Yes</Badge>
                      : di.whois_privacy === false
                    ...
                    <div className="d-flex flex-column gap-2">
                      {di.name_servers.map((ns, i) => (
                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.375rem 0.75rem', borderRadius: 6, background: 'var(--tl-bg-surface)', fontFamily: 'var(--tl-font-mono)', fontSize: '0.8rem', color: 'var(--tl-text-secondary)' }}>
                          <ChevronRight size={12} color="var(--tl-primary-light)" />
                          {ns}
                        </div>
                      ))}
                    </div>
                  ) : (
                    <span style={{ color: 'var(--tl-text-faint)', fontSize: '0.875rem', fontStyle: 'italic' }}>
                      {di.whois_error || 'No name servers found'}
                    </span>
                  )}
                    ...
                    <>
                      ...
                      <div className="d-flex flex-wrap gap-2">
                        {di.whois_status.slice(0, 5).map((s, i) => (
                          <Badge key={i} variant="outline" style={{ fontFamily: 'var(--tl-font-mono)', fontSize: '0.7rem' }}>
                            {s.split(' ')[0]}
                          </Badge>
                        ))}
                      </div>
              ...
              <div className="col-12">
                <div className="tl-card p-4">
                  ...
                  {result?.dns?.error ? (
                    <div style={{ color: 'var(--tl-danger)', fontSize: '0.875rem' }}>
            ...
            <div className="row g-4">
              <div className="col-12 col-lg-6">
                <SectionCard
                  ...
                  icon={<Lock size={16} color="var(--tl-primary-light)" />}
                  error={!si.valid && si.ssl_error ? si.ssl_error : null}
                >
                  <InfoRow label="Status">
                    {si.valid
                      ? <Badge variant="success"><CheckCircle size={11} style={{ marginRight: 4 }} />Valid</Badge>
                  ...
                  <InfoRow label="Days Remaining">
                    {si.days_remaining != null
                      ? <span style={{ fontWeight: 700, color: si.days_remaining < 30 ? 'var(--tl-danger)' : si.days_remaining < 90 ? 'var(--tl-warning)' : 'var(--tl-success)' }}>
                          {si.days_remaining} days
                        </span>
                      : val(null)}
                  </InfoRow>
                  {si.signature_algorithm && (
                    <InfoRow label="Sig. Algorithm">{val(si.signature_algorithm)}</InfoRow>
                  )}
                </SectionCard>
              </div>
              ...
              <div className="col-12 col-lg-6">
                <div className="tl-card p-4 h-100">
                  ...
                  {si.sans?.length > 0 ? (
                    ...
                    <span style={{ color: 'var(--tl-text-faint)', fontStyle: 'italic', fontSize: '0.875rem' }}>
                      {si.ssl_error || 'No SANs available'}
                    </span>
                  )}
                </div>
              </div>
                ...
                <div className="d-flex flex-column gap-2">
                  {result.redirect_chain.map((redirect, i) => (
                    <div key={i} className="d-flex align-items-center gap-2" style={{ color: 'var(--tl-text-secondary)', fontSize: '0.875rem' }}>
                      ...
                      <Badge variant={redirect.status_code === 200 ? 'success' : redirect.status_code ? 'warning' : 'outline'}>
                ...
                {result?.timeline?.map((item, i) => (
                  <div key={i} className="d-flex gap-3" style={{ marginBottom: '1.25rem', position: 'relative' }}>
                    ...
                    <div>
                      ...
                      <div style={{ color: 'var(--tl-text-faint)', fontSize: '0.75rem', marginTop: 2 }}>
                        {formatDate(item.time) || item.time}
                      </div>
                    </div>
                  </div>
                ))}
